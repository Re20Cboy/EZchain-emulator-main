...
func (bc *BlockChain) GenerateBlock() *core.Block {
	...
	//bh.StateRoot = rt.Bytes() //EZchain不需要链上状态root记录
	bh.TxRoot = GetTxTreeRoot(txs)
	bh.Bloom = *core.NewBloomFilter() //大小默认为1024*1024
	///////////////////////////
	/////// by XLD-9-18 ///////
	///////////////////////////
	b := core.NewBlock(bh)
...
func (bc *BlockChain) AddAccounts(ac []string, as []*core.AccountState) {
	...
	rt := common.BytesToHash(bc.CurrentBlock.Header.TxRoot)
	if len(ac) != 0 {
		///////////////////////////
		/////// by XLD-9-18 ///////
		///////////////////////////
		//st, err := trie.New(trie.TrieID(common.BytesToHash(bc.CurrentBlock.Header.StateRoot)), bc.triedb)
		st, err := trie.New(trie.TrieID(common.BytesToHash(bc.CurrentBlock.Header.TxRoot)), bc.triedb)
		if err != nil {
			log.Panic(err)
		}
		for i, addr := range ac {
